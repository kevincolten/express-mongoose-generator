...
function generateModel(path, modelName, modelFields, cb) {
    ft.createDirIfIsNotDefined(path, 'models', function () {
        ...
        var schemaName = formatTools.firstLower(modelName) + 'Schema';
...
function generateRouter(path, modelName, cb) {
    ft.createDirIfIsNotDefined(path, 'routes', function () {
        ...
        ft.writeFile(path + '/routes/' + modelName.toLowerCase() + 's.js', router, null, cb);
    });
}
...
function generateController(path, modelName, modelFields, cb) {
    ft.createDirIfIsNotDefined(path, 'controllers', function () {
        ...
        modelFields.forEach(function (f, index, fields) {
            ...
            updateFields += formatTools.firstLower(modelName) + '.' + field + ' = req.body.' + field + ' ? req.body.' + field + ' : ' +
                formatTools.firstLower(modelName) + '.' + field + ';';
            updateFields += '\r\t\t\t';
        ...
        controller = controller.replace(/{name}/g, formatTools.firstLower(modelName));
        controller = controller.replace(/{pluralName}/g, formatTools.firstLower(formatTools.pluralize(modelName)));
